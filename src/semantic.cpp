            ...
            if (f->kind() == ast::STMT_EXPR) {
                ...
                if (w->inherit != nullptr) {
                    error(exp::ENUMERATION,
                          "enumeration type cannot be inherited", w->name.line);
                }
                ...
                std::replace(std::begin(*statements), std::end(*statements),
                             now(), n);
                std::cout << "\033[33m[Semantic analysis replace "
