...
void Compiler::stmt(ast::Stmt *stmt) {
    switch (stmt->kind()) {
        ...
        case ast::STMT_FUNC: {
            ...
            this->now =
                this->entities.at(entitiesSize); // restore to main entity
        ...
        case ast::STMT_WHOLE: {
            ...
            if (w->inherit != nullptr) {
                ast::InheritStmt *i =
                    static_cast<ast::InheritStmt *>(w->inherit);
            ...
            for (auto i : w->body->block) {
                ...
                if (i->kind() == ast::STMT_INTERFACE) {
                    ast::InterfaceStmt *inter =
                        static_cast<ast::InterfaceStmt *>(i);
        ...
        case ast::STMT_ENUM: {
            ...
            for (int i = 0; i < e->field.size(); i++) {
                obj->elements.insert(
                    std::make_pair(i, e->field.at(i)->literal));
            }
