...
namespace lexer {
    ...
    void Lexer::lexString(bool longStr) {
        ...
        while (!isEnd()) {
            ...
            if (now() == '\n' && !longStr) {
                throw exp::Exp(exp::STRING_EXP,
                               // long strings
                               "for long strings use the ` operator",
                               this->line);
                break;
            }
            literal << now();
        ...
        if (!isEndFile)
            throw exp::Exp(exp::STRING_EXP, "missing closing symbol",
                           this->line);
...
#define S_BOOL "bool"   // 5
    ...
    class BinaryExpr : public Expr {
      public:
        ...
        explicit BinaryExpr(Expr *l, token::Token op, Expr *r)
            : left(l), right(r) {
            this->op = std::move(op);
        }
    ...
    class GetExpr : public Expr {
      public:
        ...
        std::string stringer() override {
            return "<GetExpr { Expr=" + expr->stringer() + " Get='" +
                   name.literal + "' }>";
        }
    ...
    class SetExpr : public Expr {
      public:
        ...
        explicit SetExpr(Expr *e, token::Token name, Expr *v)
            : expr(e), value(v) {
            this->name = std::move(name);
        }

        std::string stringer() override {
            return "<SetExpr { Expr='" + expr->stringer() + " Name='" +
                   name.literal + "' Value=" + value->stringer() + " }>";
        }
    ...
    class ArrayExpr : public Expr {
      public:
        ...
        explicit ArrayExpr(std::vector<Expr *> e) {
    ...
    class NewStmt : public Stmt {
      public:
        ...
        explicit NewStmt(token::Token name,
                         std::map<token::Token *, Expr *> b) {
            this->name = std::move(name);
            this->builder = b;
        }
    ...
    class AndStmt : public Stmt {
      public:
        ...
        std::string stringer() override {
            return "<AndStmt Name='" + name.literal +
    ...
    class CallInheritStmt : public Stmt {
      public:
        ...
        explicit CallInheritStmt(int line, Expr *e) : expr(e) {
            this->line = line;
        }
...
namespace parser {
    ...
    ast::Expr *Parser::assignment() {
        ...
        if (look(token::EQ)) {
            ...
            // EXPR = EXPR
            if (expr->kind() == ast::EXPR_NAME ||
                expr->kind() == ast::EXPR_INDEX) {
    ...
    ast::Expr *Parser::comparison() {
        ...
        while (look(token::GREATER) || look(token::GR_EQ) ||
               look(token::LESS) || look(token::LE_EQ)) {
            token::Token op = this->previous();
    ...
    ast::Stmt *Parser::stmt() {
        ...
        case token::DEF:
            ...
            // function or interface
                ...
                while (!look(token::R_PAREN)) {
                    if (!look(token::IDENT)) {
                        error(exp::UNEXPECTED,
                              "argument name muse be an identifier");
                    }
                    // K
                ...
                return new ast::WholeStmt(previous(), inherit,
                                          this->block(token::END));
            }
            break;
            // if
        ...
        case token::NEW: {
            ...
            while (true) {
                ...
                /**
                 * variable on the stack cannot be used within a
                 * loop
                 *
                 * the location where the current token is
                 * stored
                 */
                ...
                builder.insert(
                    ...
                    std::make_pair(&this->tokens.at(tempPosition),
                                   this->expr()));
                ...
                error(exp::UNEXPECTED,
                      "expect ',' of '}' after field of class");
            }
            return new ast::NewStmt(name, builder);
        } break;
            // and
        ...
        case token::USE: {
            ...
            if (!look(token::IDENT)) {
                error(exp::UNEXPECTED,
                      "use of module name must be an identifier");
            }
            // name
            token::Token name = previous();
            // no alias
            ...
            if (!look(token::IDENT)) {
                error(exp::UNEXPECTED,
                      "alias of module name must be an identifier");
            }
        ...
        case token::L_ARROW: {
            ...
            if (!look(token::IDENT)) {
                error(exp::UNEXPECTED,
                      "inheritance name must be an indentifier");
            }
            std::vector<token::Token *> names;
            // single
      ...
      public:
                ...
                case ast::STMT_PUB: {
                    ...
                    if (next != nullptr) {
                        ...
                        case ast::STMT_INTERFACE:
                            break;
                        default:
                            error(exp::CANNOT_PUBLIC,
                                  "statement cannot be public", stmt->line);
                        }
                    }
                } break;
                case ast::STMT_WHOLE: {
                    ...
                    if (f->kind() == ast::STMT_EXPR) {
                        ...
                        if (stmt->inherit != nullptr) {
                            error(exp::ENUMERATION,
                                  "enumeration type cannot be inherited",
                                  stmt->name.line);
                        }
                        ...
                        for (auto &i : stmt->body->block) {
                            ...
                            if (i->kind() != ast::STMT_EXPR) {
                                error(exp::ENUMERATION,
                                      "whole is an enumeration type",
                                      stmt->name.line);
                            }
                            ast::ExprStmt *pStmt =
                                static_cast<ast::ExprStmt *>(i);
                            ...
                            ast::NameExpr *name =
                                static_cast<ast::NameExpr *>(pStmt->expr);
                            // push to enumeration
                            // structure
                        ...
                        std::replace(std::begin(*statements),
                                     std::end(*statements), now(), n);
                ...
                case ast::STMT_CALLINHERIT: {
                    ...
                    if (stmt->expr->kind() != ast::EXPR_CALL) {
                        error(exp::CALL_INHERIT,
                              "only methods of the parent class can be called",
                              2);
                    }
                } break;
                default:
        ...
        void analysisExpr(ast::Expr *expr) {
            ...
            case ast::EXPR_BINARY: {
                ...
                Token l =
                    (static_cast<ast::LiteralExpr *>(binary->left))->token;
                Token r =
                    (static_cast<ast::LiteralExpr *>(binary->right))->token;
                ...
                case AS_SUB: // -=
                    if (l.kind == NUM) {
                        ...
                        if (r.kind == STR || r.kind == CHAR) {
                            error(exp::TYPE_ERROR, "unsupported operand",
                                  l.line);
                        }
                    }
                ...
                case AS_DIV: // /=
                    ...
                    if (r.kind == NUM) {
                        ...
                        if (std::stof(r.literal) == 0) {
                            error(exp::DIVISION_ZERO, "division by zero",
                                  l.line);
                        }
                    }
                    // array
...
namespace entity {
    ...
    struct Entity {
        ...
        std::vector<std::string> variables;      // variables
        std::vector<ast::Type *> types;          // type of variables

        // return whether the variables of an entity is contains a varname
        bool contains(std::string var) {
            return std::find(variables.begin(), variables.end(), var) !=
                   variables.end();
        }
            ...
            for (int ip = 0, op = 0; ip < codes.size(); ip++) {
                ...
                case byte::STORE: {
                    printf("%20d: %s %10d '%s' %d %s\n", ip,
                           byte::codeString[codes.at(ip)].c_str(),
                           offsets.at(op), variables.at(offsets.at(op)).c_str(),
                           offsets.at(op + 1),
                ...
                case byte::LOAD: {
                    printf("%20d: %s %11d '%s'\n", ip,
                           byte::codeString[codes.at(ip)].c_str(),
                           offsets.at(op++),
                           variables.at(offsets.at(op)).c_str());
                } break;
                case byte::GET:
                case byte::SET: {
                    printf("%20d: %s %12d '%s'\n", ip,
...
namespace compiler {
    ...
    class Compiler {
      private:
        ...
        void emitVariable(std::string);      // push variable to entity
        void emitType(ast::Type *);          // push vars type to entity
    ...
    void Compiler::compile() {
        while (this->position < this->statements.size()) {
            ...
            switch (look()->kind()) {
            case ast::STMT_EXPR: {
                ast::ExprStmt *expr = static_cast<ast::ExprStmt *>(look());
                this->expr(expr->expr);
            } break;
            default:
                this->stmt(look());
        ...
        this->entities.push_back(this->now);
    }
    ...
    // push variable to entity
    void Compiler::emitVariable(std::string v) {
        this->now.variables.push_back(v);
    }

    // push vars type to entity
    void Compiler::emitType(ast::Type *t) { this->now.types.push_back(t); }
    ...
    void Compiler::expr(ast::Expr *expr) {
        ...
        case ast::EXPR_NAME: {
            ...
            if (now.contains(n->token.literal)) {
                ...
                // not exist
                this->emitVariable(n->token.literal);
                this->emitOffset(iVarOffset++);
            }
        ...
        case ast::EXPR_CALL: {
            ...
            for (auto i : c->arguments) this->expr(i); // arguments
        ...
        case ast::EXPR_GET: {
            ...
            this->emitVariable(g->name.literal); // name
        ...
        case ast::EXPR_SET: {
            ...
            this->emitVariable(s->name.literal); // name
        ...
        case ast::EXPR_ARRAY: {
            ...
            this->emitOffset(a->elements.size());
        } break;
        //
        case ast::EXPR_TUPLE: {
            ...
            this->emitOffset(t->elements.size());
        } break;
        //
        case ast::EXPR_MAP: {
            ...
            this->emitOffset(m->elements.size() * 2);
        } break;
        //
        case ast::EXPR_INDEX: {
    ...
    void Compiler::stmt(ast::Stmt *stmt) {
        ...
        case ast::STMT_VAR: {
            ...
            this->emitVariable(v->name.literal);
            this->emitOffset(this->iVarOffset++);
        ...
        case ast::STMT_IF: {
            ...
            if (!i->efBranch.empty()) {
                ...
                for (auto i : i->efBranch) {
                    ...
                    this->insertPosOffset(efPos, now.codes.size() +
                                                     1); // TO: ef (F_JUMP)
        ...
        case ast::STMT_FOR: {
            ...
            int original =
                now.codes.size(); // original state: for callback loops
    ...
    // search list variables to return index
    int Compiler::searchVarIndexes(std::string name) {
        std::vector<std::string>::iterator iter =
            std::find(now.variables.begin(), now.variables.end(), name);
        return std::distance(now.variables.begin(), iter);
    }
}; // namespace compiler
...
void run(std::string source) {
    try {
        ...
        compiler->compile();
        compiler->entities[0].dissemble(0);
        //
    } catch (exp::Exp &e) {
