...
namespace token {
    ...
    std::string kindString[len] = {"IDENT", "NUM", "STR",
                                   "CHAR", "FLOAT", "ADD",
                                   "SUB", "MUL", "DIV",
...
namespace byte {
    ...
    std::string codeString[len] = {"CONST", "STORE", "LOAD", "ADD",
                                   "SUB", "MUL", "DIV", "RET"};
...
namespace object {
    ...
    class Str : public Object {
        public:
            ...
            std::string stringer() override { return "<Str \"" + value + "\">"; }
...
// entity structure
    struct Entity {
        ...
        void dissemble(int p) {
            std::cout << "ENTITY(" << std::to_string(p) << "): " << std::endl;

            for (int ip = 0, cp = 0, op = 0, vp = 0; ip < codes.size(); ip++) {
                ...
                switch (co) {
                    case byte::CONST: {
                        printf("%20d: %s %10d %s\n", ip,
                               ...
                               offsets.at(op++),
                               constants.at(cp++)->stringer().c_str());
                    }
                        break;
                    case byte::LOAD: {
                        printf("%20d: %s %11d '%s'\n", ip,
...
namespace compiler {
    ...
    class Compiler {
        private:
            ...
            void emitCode(byte::Code);
            void emitOffset(int);
            void emitConstant(object::Object *);
            void emitVariable(std::string);
    ...
    void Compiler::expr(ast::Expr *expr) {
        switch (expr->kind()) {
            case ast::EXPR_LITERAL: {
                ...
                this->emitOffset(this->nowOffset++);
                break;
            }
            case ast::EXPR_BINARY: {
            ...
            case ast::EXPR_NAME: {
                ast::NameExpr *name = static_cast<ast::NameExpr *>(expr);

                this->emitCode(byte::LOAD);

                std::cout << this->now.contains(name->token.literal) << std::endl;
                //if (this->now.contains(name->token.literal)) {
                //    int off = this->searchVarIndexes(name->token.literal);
                //    this->emitOffset(off);
                //} else {
                //    this->emitVariable(name->token.literal);
                //    this->emitOffset(this->nowOffset++);
                //}
                this->emitVariable(name->token.literal);
                this->emitOffset(this->nowOffset++);
    ...
    int Compiler::searchVarIndexes(std::string var) {
        std::vector<std::string>::iterator iter = std::find(
            this->now.variables.begin(), this->now.variables.end(), var);
        return iter != this->now.variables.end() ? std::distance(this->now.variables.begin(), iter) :
               (this->nowOffset++, this->nowOffset - 1);
    }
}; // namespace compiler
